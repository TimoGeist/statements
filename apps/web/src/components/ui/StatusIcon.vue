...
import { LoaderCircle, X, Check, CircleAlert } from 'lucide-vue-next';
defineProps<{
	status: "active" | "waiting" | "failed" | "completed" | true | false
	size: number
}>()
</script>
...
<template>
	<template v-if="status === 'active'">
		<LoaderCircle class="animate-spin" :size="size" />
	</template>
	...
	<template v-if="status === null">
		<span class="text-xl flex items-center justify-center font-bold antialiased">?</span>
	</template>
	...
	<template v-if="status === 'waiting'">
		<svg class="text-foreground" :width="size" :height="size" viewBox="0 0 32 32" id="icon" fill="currentColor"
			xmlns="http://www.w3.org/2000/svg">
			<rect fill="currentColor" class="cls-1" x="22" y="20" width="8" height="2" />
			<rect fill="currentColor" class="cls-1" x="22" y="24" width="8" height="2" />
			<rect fill="currentColor" class="cls-1" x="22" y="28" width="8" height="2" />
			<rect fill="currentColor" class="cls-1" x="18" y="24" width="2" height="2" />
			<rect fill="currentColor" class="cls-1" x="18" y="20" width="2" height="2" />
			<rect fill="currentColor" class="cls-1" x="18" y="28" width="2" height="2" />
			...
			<path fill="currentColor" class="cls-1"
				...
				transform="translate(0 0)" />
			<rect fill="none" id="_Transparent_Rectangle_" data-name="&lt;Transparent Rectangle&gt;" class="cls-2"
				width="32" height="32" />
		</svg>
	</template>
</template>
